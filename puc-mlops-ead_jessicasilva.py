...
def reset_seeds():     #função para resetar as sementes
   os.environ['PYTHONHASHSEED']=str(42)
   tf.random.set_seed(42)
   np.random.seed(42)
   random.seed(42)

def read_data():     #função para ler e preparar o dataset
...
def process_data(X, y):     #função para processamento dos dados
...
def create_model(X):    #função para criar e configurar o modelo
   reset_seeds()
   model = Sequential()
...
def config_mlflow():     #configurar o mlflow
...
def train_model(model, X_train, y_train, is_train=True):     #função para treinar o modelo
